...
const cards = await p.evaluate(() => [...document.querySelectorAll("[data-lead-id]")].map((c) => [c.querySelector(".row-title").textContent.trim(), (c.querySelector(".row-contract") || {}).textContent || ""]));
console.log("cards:", JSON.stringify(cards));
const dana = cards.find((c) => c[0] === "Dana Muise"), ravi = cards.find((c) => c[0] === "Ravi Anand"), lynn = cards.find((c) => c[0] === "Lynn Chu");
if (!dana || !/\$532\/mo/.test(dana[1]) || !/payments left/.test(dana[1]) || !/matures/.test(dana[1])) fail("Dana's card doesn't say what she pays and how many are left: " + JSON.stringify(dana));
if (!ravi || !/paid off/.test(ravi[1])) fail("Ravi's paid-off contract isn't on his card: " + JSON.stringify(ravi));
if (!lynn || lynn[1]) fail("a customer with no contract got a contract line: " + JSON.stringify(lynn));
...
const headline = await p.evaluate(() => document.querySelector("#view .card .row-contract")?.textContent.trim());
if (!/\$532\/mo/.test(headline || "") || !/23 payments left/.test(headline || "")) fail("the name box doesn't carry the contract line: " + headline);
