...
//
// Without a store: create one (you become its manager) or join one by
// invite code. A rep sees who's on the team and their own numbers. A manager
...
import { cachedStore, myStore, createStore, joinStore, setMemberRole, leaveStore, setMyName, isManager, inviteLink, memberName, boardStats, repLead } from "../team.js";
...
export function renderTeam(view, { param } = {}) {
  ...
  function draw() {
    ...
    el.innerHTML = `
        ...
        ${(team.members || []).map((m) => `
          <div class="row" style="padding:6px 0">
            ...
            <div class="row-meta"><span class="badge ${m.role === "manager" ? "badge-sold" : "badge-working"}">${m.role === "manager" ? "Manager" : "Rep"}</span>
              ${manager && m.user_id !== me.id ? `<button class="btn btn-ghost btn-sm" data-role="${esc(m.user_id)}" style="margin-left:6px">…</button>` : ""}</div>
          </div>`).join("")}
      ...
      <div class="card">
        ...
        <div class="hint">Invite code: <span class="mono">${esc(team.code)}</span> — a rep can also type it under Tools → Team.</div>
      </div>` : ""}
    `;
  ...
  function drawNoStore() {
    el.innerHTML = `
      ...
      <div class="section-title">I'm a manager</div>
      <div class="card">
        ...
        <div class="hint">You become its first manager and get an invite link for the reps.</div>
      </div>
    ...
    `;
    el.querySelector('[data-act="create"]').addEventListener("click", async (ev) => {
      ...
      try { team = await createStore(name, meName); error = ""; toast(`${name} is set up — you're its manager`, "success"); draw(); refreshBoard(); }
  ...
  function openRoleSheet(m) {
    if (!m) return;
    openModal(memberName(m), (close) => {
      const root = document.createElement("div");
      root.innerHTML = `
        <div class="small muted" style="margin-bottom:10px">${esc(m.email || "")}</div>
        <button class="btn btn-ghost btn-block" data-r="${m.role === "manager" ? "rep" : "manager"}" style="margin-bottom:8px">${m.role === "manager" ? "Make a rep" : "Make a manager"}</button>
        <button class="btn btn-danger btn-block" data-r="remove">Remove from the store</button>
        <div class="hint">A manager sees everyone's numbers and customers. Removing someone leaves their own book untouched — they just drop off the board.</div>`;
      root.querySelectorAll("[data-r]").forEach((b) => b.addEventListener("click", async () => {
        const role = b.dataset.r;
        if (role === "remove" && !(await confirmDialog(`Remove ${memberName(m)} from ${team.name}?`, { confirmLabel: "Remove" }))) return;
        try { team = await setMemberRole(m.user_id, role); close(); draw(); if (board) refreshBoard(); } catch (e) { toast(e.message || "Couldn't change that", "danger"); }
      }));
      return root;
    });
  }
