...
const ROWS = [
  ...
  { key: "drive", label: "Drivetrain", get: (v) => v.drive || v.drivetrain || v.transmission || "—" },
    ...
    wrap.querySelector("#ai-go").addEventListener("click", async () => {
      ...
      try {
        ...
        const res = await fetch(url, {
          ...
          body: JSON.stringify({
            system: 'Return ONLY a JSON object (no prose, no code fences) describing the requested vehicle for the Canadian market, base trim, with keys: label (string, year make model), msrp (number, CAD), engine (string), hp (number), fuel (number, combined L/100km), drive (string), seats (number), cargo (number, litres, or null), tow (number, lb, or null), warranty (string), features (string, the 2-3 standout features). Approximate values are fine.',
