...
// --- Swipe to reveal ---
// iOS-style: wrap a list card/row so sliding it left reveals buttons behind
// it. Delete is the red one on the far right; `actions` are any others, laid
// out to its left in order. Returns the wrapper to append in place of the
// element. Vertical scrolling is untouched (the gesture is claimed only once
// the movement is clearly horizontal).
//
//   actions: [{ label, icon, kind: "ok", onTap(api) }]
//   api.close()        slide the row back and put the first tray back
//   api.expand(list)   swap the tray for a second set of buttons — a follow-up
//                      question, like which way — and reveal all of them
//   api.wrap           the wrapper, for redrawing the row afterwards
//
// Everything a follow-up needs lives in the tray on purpose: it is outside the
// sliding element, so none of the drag handling can swallow a tap on it.
...
export function swipeable(el, { onDelete, label = "Delete", actions = [] } = {}) {
  ...
  const first = actions.map((a) => ({ btn: button(a), onTap: a.onTap }));
  const del = document.createElement("button");
  del.type = "button";
  del.className = "swipe-act swipe-del";
  del.innerHTML = `${icon("trash")}<span>${escapeText(label)}</span>`;
  const firstTray = () => tray.replaceChildren(...first.map((a) => a.btn), del);
  firstTray();
  ...
  const api = {
    ...
    wrap,
    expand(list) {
      tray.replaceChildren(...list.map((a) => {
        const b = button(a);
  ...
  first.forEach(({ btn: b, onTap }) => b.addEventListener("click", () => { if (onTap) onTap(api); }));
